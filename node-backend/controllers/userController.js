...
exports.apiGetReviewsByUsername = async function (req, res) {
  try {
    let authorDoc = await User.findByUsername(req.params.username)
    let reviews = await Review.findByAuthorId(authorDoc._id)
    //res.header("Cache-Control", "max-age=10").json(posts)
    res.json(reviews)
  } catch (e) {
    res.status(500).send("Sorry, invalid user requested.")
...
exports.sharedProfileData = async function (req, res, next) {
  ...
  let ReviewCountPromise = Review.countReviewsByAuthor(req.profileUser._id)
  let followerCountPromise = Follow.countFollowersById(req.profileUser._id)
  let followingCountPromise = Follow.countFollowingById(req.profileUser._id)
  let [postCount, followerCount, followingCount, reviewCount] = await Promise.all([postCountPromise, followerCountPromise, followingCountPromise, ReviewCountPromise])

  req.postCount = postCount
  req.followerCount = followerCount
  req.followingCount = followingCount
  req.reviewCount = reviewCount
...
exports.profileBasicData = function (req, res) {
  res.json({
    ...
    counts: { postCount: req.postCount, followerCount: req.followerCount, followingCount: req.followingCount, reviewCount: req.reviewCount }
  })
}
